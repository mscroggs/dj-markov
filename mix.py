...
os.system(f"mkdir -p out/{config.mode}")

with open(f"data/{config.mode}/mixability") as f:
    mixability = json.load(f)
with open(f"out/{config.mode}/data.json", "w") as f:
    pass
...
with open(f"out/{config.mode}/info.json", "w") as f:
    json.dump(info, f)
...
for n, (song1, song2, score) in enumerate(to_mix):
    print(f"[{n}/{len(to_mix)}] Mixing {song1} and {song2}")
    m = mashup.Mixer(song1, song2)
    m.load_songs()
    m.analyse(False)
    m.mix()
    m.export(f"out/{config.mode}/{n}.mp3")
    data = {
        "filename": f"out/{config.mode}/{n}.mp3",
        "song1": song1,
        "song2": song2,
    ...
    with open(f"out/{config.mode}/data.json", "a") as f:
        json.dump(data, f)
        f.write("\n")
    n += 1
